...
// fetch('lib/cidades.json', { "method": 'GET', "headers": { 'Accept': 'application/json' } })
//     .then((response) => response.json())
//     .then((cidades) => {

//         cidades.forEach(function (municipio) {

//             municipios[municipio.codigoIbge] = {
//                 'nomeCidade': municipio.nomeCidade,
//                 'uf': municipio.uf
//             }
...
function salvarContatosDB() {
    ...
    return new Promise((resolve, reject) => {
        ...
        contatoModel.listar()
            .then((contatos) => {
                ...
                contatosDB.forEach(function (contato) {
                    ...
                    contato.id_usuario_ultima_sincronizacao = usuario.codigo

                });

                contatosDB = JSON.stringify(contatosDB);
        ...
        setTimeout(() => {
            ...
            jQuery.ajax({
                url: '/cadastros/orm/Contato.php',
                data: { 'modulo': 'Contato', 'operacao': 'insertContato', 'contatos': contatosDB },
                type: 'POST',
                async: true,
                dataType: 'json',

                success: function (errosSinc) {
                    getErrosSinc(errosSinc)
                    resolve;
...
function getErrosSinc(erros){
    errosSincronizacao = erros
    return erros
}
